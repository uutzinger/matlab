function [EEM,alpha_ex]=assembleSPXDX_room_fix(StartDir, FilePattern, varargin);
...
if em_count == 0
    if Type ~= 2
     if bg_t ~= 0
       if ex<=455 & ex>=420 %%%spectral measurement between 420 and 5600 nm
     [ws,is] = sort(w);  %%%ensure all wavelengths are in ascending order
     S_s = S(is);
     R_s = R(is);
     %%%fingd range of spectra at least 4 times above 
     bg_i = find(bg_room(:,1)==ws(7)); %%%index of background cutting out excitation tail
     bg_j = find(bg_room(:,1)==650);
     S_si = find(ws == ex+35);  %%%index for signal
     S_sj = find(ws == 650);
     indx1_b = find(S_s(S_si:S_sj)>(4.*bg_room(bg_i:bg_j,2)))+6;  %index below 650 nm with spectra at least 4 times background
     %indx1_b = find(ex+45<=ws & ws<=650); %lower values of spectra used to find alpha
     indx1_bg_b = find(ws(indx1_b(1))<=bg_room(:,1) & bg_room(:,1)<=ws(indx1_b(end)));
     ...
     em_count = 1;
        ...
        S_s = S(is);
        R_s = R(is);
        indx2_b = indx1_b-1; %%shift index to ensure same wavelengths are being usesd
        indx2_bg_b = indx1_bg_b;  %%no change in background measurement
        indx2_a = find(650<ws); % values of spectra above 650 used to find background
        m2_b = S_s(indx2_b);
        p2_b = R_s(indx2_b);
        m2_a = S_s(indx2_a);
        p2_a = R_s(indx2_a);
        ...
        %sp_b = (m1_b-bg_b);  %%calculate spectra based on generated equations %%without power 
        sp_b = (m1_b-bg_b)./p1_b;  %%calculate spectra based on generated equations (see bgextract.m for equations)
        figure(1); %hold on;
        plot(ws(indx2_b)-5,sp_b); title(strcat(num2str(ex),' Ex Spectra below 650'));
        pause
        %alpha = (m2_b-bg_b)./(sp_b);  %%without power
        alpha = (m2_b-bg_b)./(p2_b.*sp_b);
        
        figure(2); %hold on; 
         plot(alpha); title(strcat('Alpha value', num2str(ex),' Ex'));
          pause
        
        alpha_m = mean(alpha);
        alpha_ex = [alpha_ex,alpha_m];
            
        %sp_a = (m1_a-m2_a)./(1-alpha_m);  %%solve for new spectra above 650 nm without power
        sp_a = (m1_a-m2_a)./(p1_a-alpha_m.*p2_a);  %%solve for new spectra above 650 nm
        figure(3); plot(ws(indx2_a),sp_a); title(strcat(num2str(ex),' Ex: Spectra above 650'));
        pause
        %bg_a_temp = m1_a - sp_a; %%%calculated background without power
        bg_a_temp = m1_a - sp_a.*p1_a; 
        %(p1_a.*m2_a-m1_a*alpha_m.*p2_a)./(p1_a-alpha_m.*p2_a);
        
        figure(4); hold on;
        plot(ws(indx2_a),bg_a_temp); title(strcat(num2str(ex),' Ex: Background above 650')); 
        pause
        if min(bg_a_temp) <= 0 %%%decide whether to keep background measurmenet
         bg_a = bg_a;
        else
       bg_a = [bg_a,bg_a_temp];
...
em_count = 0;
i = i-1; %%start anew at 2 excitation wavelength
        end
        end
        end
end %%end else if em_count
end %%%end if em_count
 i = i+1;
end  %end while
