...
% 4) assembles EEM %%%%% Creates constant emission wavelength interval if
...
while i<=k,  
    ...
    if bg_t==0   % was is a background measurement
       bg=[bg; [ex, mean(S)]]; % save average bg and excitation wavel.
           ...
           interv=abs(round((w(end)-w(1))/(length(w)-1))); % generate integer wavelength interval
           w_int=interv*round(w./interv); % round wavelength to new interval
           d_int=interp1(w,(S./R),w_int,'nearest')'; % interpolate data to new wavelength vector
            if(w(1)>w(end))
                delta = round(w_int(end)-w(end));
            else
                delta = round(w_int(1)-w(1));
            end
           for j = 1:1:length(w_int)
               w_int(j) = w_int(j) - delta;
           end
           EEM(i,w_int)=d_int; % fill up the EEM device signal by lamp power
           EEM(i,1)=ex;
       end
    end
    i=i+1;
